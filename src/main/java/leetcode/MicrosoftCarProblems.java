...
public class MicrosoftCarProblems {
  ...
  public static List<ArrayList<ArrayList<Integer>>> getAllSolution(
    ...
    int[] begins = new int[list.size()]; // the begin index of each car segments
    begins[0] = 0;
    ...
    ArrayList<ArrayList<Integer>> fristList = copayList(list);
    ...
    // others kind
    for (int i = 1; i <= n; i++) {
      List<ArrayList<Integer>> tempList = copayList(list);
  ...
  public static ArrayList<ArrayList<Integer>> insertCar(
    ...
    for (int i = 1; i <= lastSegment; i++) {
      if (index > begins[i - 1] && index <= begins[i]) { // between begins[i - 1] and begins[i]
        list.get(i - 1).add(index - begins[i - 1], Integer.MAX_VALUE);
        break;
      } else if (index > begins[lastSegment]) { // in the last segment
        list.get(lastSegment).add(index - begins[lastSegment], Integer.MAX_VALUE);
        break;
      }
    }
  ...
  public static ArrayList<ArrayList<Integer>> copayList(ArrayList<ArrayList<Integer>> list) {
